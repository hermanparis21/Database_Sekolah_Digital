...
def show_auth():
    # Header Utama (Hanya muncul di sini sekali)
...
def show_dashboard():
    ...
    # --- HOME ---
    if choice == "🏠 Home":
        st.subheader("Informasi & Notifikasi")
        ...
        if u['role'] == "Siswa":
            # 1. Notifikasi Validasi/Peringatan Foto
            df_notif = load_data("audit_notif", ttl="2s")
            if not df_notif.empty:
                my_n = df_notif[df_notif['nama'] == u['nama']].tail(3)
                for _, n in my_n.iterrows():
                    color = "#fee2e2" if "⚠️" in n['pesan'] else "#dcfce7"
                    border = "#ef4444" if "⚠️" in n['pesan'] else "#22c55e"
                    st.markdown(f"<div style='background:{color}; border-left:5px solid {border}; padding:10px; margin-bottom:10px;'>{n['pesan']}</div>", unsafe_allow_html=True)

            # 2. Pengumuman Broadcast
            df_bc = load_data("broadcast", ttl="2s")
            if not df_bc.empty:
                # Filter broadcast untuk semua kelas atau kelas spesifik siswa
                my_bc = df_bc[(df_bc['target'] == "Semua Kelas") | (df_bc['target'] == u['kelas'])].tail(3)
                for _, b in my_bc.iterrows():
                    st.info(f"📢 **{b['judul']}** (Oleh: {b['pic']})\n\n{b['pesan']}")
    ...
    # --- SPP ---
        ...
        if u['role'] == "Admin TU":
            ...
            st.subheader("Riwayat Pembayaran & Tunggakan")
            ...
            with tab_lun:
                ...
                else: st.info("Belum ada riwayat lunas.")

    # --- BROADCAST ---
    elif choice == "📢 Kirim Broadcast":
        st.subheader("Kirim Pengumuman")
        with st.form("bc_f"):
            jd = st.text_input("Judul Broadcast")
            msg = st.text_area("Isi Pesan")
            trg = st.selectbox("Target Kelas", ["Semua Kelas"] + list_kelas)
    ...
    # --- LAPORAN & AUDIT ---
        ...
        with t1:
            if not df_p.empty:
                ...
                if not u_ref.empty and not u_abs.empty:
                    ...
                    c2.image(f"data:image/jpeg;base64,{u_abs.iloc[0]['foto_absen']}", caption="Foto Saat Absen")
        ...
        with t2:
            ...
            f_tgl = col_f1.date_input("Filter Tanggal", value=now.date())
            f_kls = col_f2.selectbox("Filter Kelas", ["Semua"] + list_kelas)
            ...
            if f_kls != "Semua":
                df_filtered = df_filtered[df_filtered['kelas'] == f_kls]
            
            def style_terlambat(row):
                return ['color: red; font-weight: bold' if row.status == 'Terlambat' else '' for _ in row]
            
            st.dataframe(df_filtered.style.apply(style_terlambat, axis=1), use_container_width=True)
