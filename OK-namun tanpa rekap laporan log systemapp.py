...
# Fungsi muat data dengan proteksi kuota (Hanya untuk Dashboard)
@st.cache_data(ttl=5)
def load_data_cached(sheet_name):
    return conn.read(worksheet=sheet_name)

# Fungsi muat data LIVE (Khusus Login & Submit Data)
...
# --- 5. AUTHENTICATION (FIXED: ALWAYS LIVE) ---
def show_auth():
    ...
    with tab1:
        with st.form("l_form"):
            ...
            if st.form_submit_button(L['login']):
                df_u = load_data_live("users") # Pastikan baca data terbaru
                ...
                else: st.error("Login Gagal! Pastikan nama dan password benar.")
    with tab2:
        with st.form("r_form"):
            ...
            if st.form_submit_button(L['reg']):
                ...
                conn.update(worksheet="users", data=pd.concat([df_u, new_u], ignore_index=True)); st.success("Registrasi Berhasil! Silakan Masuk di Tab Sebelah.")
...
def show_dashboard():
    ...
    if choice == "🏠 Home":
        ...
        if user['role'] in ["Guru", "Admin TU"]:
            ...
            tunggakan = 1200000 - df_spp[df_spp['nama'] == user['nama']]['jumlah'].sum() if not df_spp.empty else 1200000
    ...
    # --- PRESENSI ---
        ...
        if loc:
            ...
            else: st.error(f"Di luar radius! Jarak: {int(dist)}m")

    # --- TUGAS ---
    elif choice == f"{L['tugas']}":
        st.header(L['tugas'])
        df_t = load_data_cached("tugas"); df_done = load_data_cached("tugas_selesai")
        if user['role'] == "Guru":
            with st.expander("➕ Buat Tugas Baru"):
                with st.form("t_f"):
                    ...
                    if st.form_submit_button("Kirim"):
                        df_t_live = load_data_live("tugas")
                        new_t = pd.DataFrame([{"id": datetime.now(jakarta_tz).strftime("%H%M%S"), "guru": user['nama'], "judul": t_j, "deskripsi": t_d, "kelas": ",".join(t_k)}])
                        conn.update(worksheet="tugas", data=pd.concat([df_t_live, new_t], ignore_index=True)); st.cache_data.clear(); st.rerun()
            st.divider()
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer: df_done.to_excel(writer, index=False)
            st.download_button("📥 Ekspor Tugas (XLSX)", output.getvalue(), "tugas.xlsx")

        else: # SISWA
            user_kls = str(user.get('kelas',''))
            rel_t = df_t[df_t['kelas'].astype(str).str.contains(user_kls, na=False)] if not df_t.empty else pd.DataFrame()
            for _, r in rel_t.iterrows():
                is_comp = not df_done[(df_done['id_tugas'].astype(str) == str(r['id'])) & (df_done['nama'] == user['nama'])].empty if not df_done.empty else False
                ...
                if not is_comp:
                    if st.button(f"Klik Selesai: {r['judul']}", key=f"btn_{r['id']}"):
                        df_live = load_data_live("tugas_selesai")
                        new_d = pd.DataFrame([{"id_tugas": str(r['id']), "nama": user['nama'], "kelas": user['kelas'], "waktu": now_dt.strftime("%d/%m %H:%M")}])
                        conn.update(worksheet="tugas_selesai", data=pd.concat([df_live, new_d], ignore_index=True)); st.cache_data.clear(); st.rerun()
                else: st.success("✅ Tugas Selesai")

    # --- LAPORAN GURU (FIXED & STYLED) ---
    elif choice == L['lapor']:
        st.header(L['lapor'])
        df_p = load_data_live("presensi")
        if not df_p.empty:
            ...
            st.download_button("📥 Ekspor XLSX", buffer.getvalue(), "laporan_presensi.xlsx")
            st.download_button("📥 Ekspor TXT", df_p.to_string(index=False), "laporan_presensi.txt")
        ...
        if user['role'] == "Admin TU":
            with st.form("spp"):
                ...
                if st.form_submit_button("Bayar"):
                    ...
                    conn.update(worksheet="spp", data=pd.concat([df_live, new_p], ignore_index=True)); st.cache_data.clear(); st.success("Lunas!")
