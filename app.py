...
# Logika Waktu Laporan Guru (Sesuai Permintaan Sebelumnya)
...
def show_dashboard():
    ...
    # --- KEMBALI KE FITUR ABSENSI MANTAP SEBELUMNYA ---
    ...
    # --- LAPORAN GURU (DENGAN PENYEMPURNAAN REQ SEBELUMNYA) ---
    elif choice == L['lapor']:
        st.header(L['lapor'])
        df_p = load_data("presensi", ttl="5s")
        if not df_p.empty:
            ...
            st.download_button("📥 Ekspor XLSX", buf.getvalue(), "laporan_presensi.xlsx")
            st.download_button("📥 Ekspor TXT", df_p.to_string(index=False), "laporan_presensi.txt")

    # --- SPP (LOGIKA MANUAL ADMIN TU) ---
    elif choice == L['spp']:
        st.header(L['spp'])
        ...
        if user['role'] == "Admin TU":
            with st.form("f_spp"):
                ...
                ket = st.text_input("Keterangan (Contoh: Tunggakan SPP Jan-Feb)")
            ...
            st.divider()
            st.subheader("Rekap Seluruh Siswa")
            st.dataframe(df_spp)
        else:
            st.subheader(f"Data Tagihan: {user['nama']}")
            # SISWA HANYA MELIHAT HASIL INPUT ADMIN TU
            my_spp = df_spp[df_spp['nama'] == user['nama']] if not df_spp.empty else pd.DataFrame()
            if not my_spp.empty:
                st.table(my_spp[['tanggal', 'keterangan', 'jumlah', 'status']])
            else:
                st.warning("Belum ada data tagihan/pembayaran resmi dari Admin TU.")
