...
lang_dict = {
    "ID": {
        ...
        "tugas": "📚 Tugas Sekolah", "lapor": "📊 Laporan Presensi", "log_sys": "⚙️ Log System", "out": "Keluar"
    },
    "EN": {
        ...
        "absen_h": "📍 Face & GPS Attendance", "pilih_j": "Attendance Type", "face_now": "Verify Face Now",
        "tugas": "📚 Tasks", "lapor": "📊 Attendance Reports", "log_sys": "⚙️ Log System", "out": "Logout"
    }
}
        ...
        with st.form("l_form"):
            ...
            if st.form_submit_button(L['login']):
                ...
                if not m.empty:
                    ...
                    add_log(u, "LOGIN", "Berhasil masuk")
                    st.rerun()
...
def show_dashboard():
    ...
    if choice == f"📍 {L['absen_h']}":
        ...
        if loc:
            ...
            if dist <= 100:
                st.success(f"Lokasi Terverifikasi ({int(dist)}m)")
                ...
                if st.button("Submit Attendance") and img_now:
                    ...
                    add_log(user['nama'], "PRESENSI", f"{m_absen} berhasil")
                    st.balloons(); st.success("Presensi Berhasil!")
            else: st.error(f"{L['gps_fail']} Jarak: {int(dist)}m")

    # --- TUGAS (MONITORING & PROGRESS) ---
        ...
        if user['role'] == "Guru":
            ...
            st.subheader("📈 Monitoring Progress Tugas")
            for _, r in df_t.iterrows():
                ...
                persen = (sudah_s / total_s * 100) if total_s > 0 else 0
                with st.expander(f"📊 {r['judul']} ({sudah_s}/{total_s} Siswa)"):
                    st.progress(persen / 100)
                    st.write(f"Persentase: **{int(persen)}%**")
    ...
    # --- LAPORAN PRESENSI (FIXED: KEMBALI DIMUNCULKAN) ---
    elif choice == L['lapor']:
        st.header(L['lapor'])
        df_p = load_data("presensi")
        f_kls = st.multiselect("Filter Kelas", list_kelas)
        f_date = st.date_input("Pilih Tanggal", value=datetime.now(jakarta_tz))
        
        filtered_df = df_p.copy()
        if f_kls: filtered_df = filtered_df[filtered_df['kelas'].isin(f_kls)]
        filtered_df = filtered_df[filtered_df['waktu'].astype(str).str.contains(str(f_date))]
        
        st.dataframe(filtered_df, use_container_width=True)
        st.download_button("📥 Download Data CSV", filtered_df.to_csv(index=False), "laporan_presensi.csv")

    # --- LOG SYSTEM ---
    elif choice == L['log_sys']:
        st.header(L['log_sys'])
        df_logs = load_data("log_system")
        st.text_area("Aktivitas Sistem", value=df_logs.sort_values(by='waktu', ascending=False).to_string(index=False), height=400)

    if st.sidebar.button(L['out']):
        add_log(user['nama'], "LOGOUT", "Keluar aplikasi")
