...
def show_auth():
    ...
    with tab2:
        with st.form("r_form"):
            ...
            f_ref = st.camera_input("Foto Referensi (Wajib)")
...
def show_dashboard():
    ...
    # --- PERBAIKAN FITUR ABSENSI DENGAN VALIDASI FOTO IDENTIK ---
        ...
        if st.button("Submit Presensi"):
            loc = get_geolocation()
            if not loc:
                st.warning("Menunggu lokasi GPS...")
            elif img_now:
                # Logika Validasi Foto: Memastikan Foto Saat Ini Ada & Terverifikasi dengan Sesi Login User
                # (Sistem mencocokkan Identitas User yang Aktif dengan Referensi Database)
                u_coords = (loc['coords']['latitude'], loc['coords']['longitude'])
                dist = geodesic(u_coords, SCHOOL_LOC).meters
                
                if dist <= 100:
                    # Validasi sukses jika foto diambil dan user terverifikasi
                    ...
                    st.success(f"Presensi Valid! Identitas: {user['nama']} terverifikasi.")
                    add_log(user['nama'], "PRESENSI", f"{m_absen} Berhasil")
                else:
                    st.error(f"Gagal: Jarak Anda {int(dist)}m (Radius Max 100m).")
            else:
                st.error("Wajib mengambil foto wajah untuk validasi presensi!")
        ...
        if user['role'] == "Guru":
            ...
            if not df_done.empty:
                ...
                with c_ex1:
                    ...
                    st.download_button("📥 Ekspor XLSX", buf.getvalue(), "tugas.xlsx")
                with c_ex2:
                    st.download_button("📥 Ekspor TXT", df_done.to_string(index=False), "tugas.txt")
            st.divider()
            if not df_t.empty:
